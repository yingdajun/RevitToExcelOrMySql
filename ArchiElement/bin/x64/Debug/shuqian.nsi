...
; 安装程序初始定义常量
!define PRODUCT_NAME "xiaoshuqian "
...
OutFile "Setup.exe"
InstallDir "$PROGRAMFILES\xiaoshuqian "
...
Push <Assembly>path</Assembly> #要替换的字符串
Push $1 #用来替换的字符串
Push all #replace all occurrences
Push all #replace all occurrences
Push C:\ProgramData\Autodesk\Revit\Addins\2018\ArchiElement.addin #所替换的文件目录
...
Section Uninstall
  ...
  Delete "$INSTDIR\about1.ico"
  Delete "$INSTDIR\beam.ico"
  Delete "$INSTDIR\column.ico"
  Delete "$INSTDIR\door.ico"
  Delete "$INSTDIR\duct.ico"
  Delete "$INSTDIR\floor.ico"
  Delete "$INSTDIR\pipe.ico"
  Delete "$INSTDIR\qiaojia.ico"
  Delete "$INSTDIR\room.ico"
  Delete "$INSTDIR\shuqianico.ico"
  Delete "$INSTDIR\wall.ico"
  Delete "$INSTDIR\window.ico"
